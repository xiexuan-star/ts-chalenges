...
type TupleToPartialUnion<T extends any[]> = T extends [...infer R, infer F] ? T | TupleToPartialUnion<R> : never;

type UnionToIntersection<T> = (T extends T ? (params: T) => any : never) extends (P: infer R) => any ? R : never;

type Curry<Args extends any[], R, U extends any[] = TupleToPartialUnion<Args>> =
  Args['length'] extends 0
    ? R
    : UnionToIntersection<U extends any
      ? (...resultArgs: U) =>
        Args extends [...U, ...infer RestArgs]
          ? Curry<RestArgs, R>
          : R
      : never>

declare function DynamicParamsCurrying<Args extends unknown[], R>( fn: (...args: Args) => R ): Curry<Args, R>
