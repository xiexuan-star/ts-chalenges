...
type PathPrefix<P extends string, K extends string | number> = P extends ''
  ? `${ K }`
  : `${ P }${ `.${ K }` | (K extends number
    ? `[${ K }]` | `.[${ K }]`
    : '') }`;

type ObjectKeyPaths<T, Prefix extends string = '', K extends keyof T = keyof T> =
  K extends string | number
    ? PathPrefix<Prefix, K> | (
    T[K] extends object
      ? ObjectKeyPaths<T[K], PathPrefix<Prefix, K>>
      : never
    )
    : never;


const foo: ObjectKeyPaths<typeof ref> = 'person.pets.0';
