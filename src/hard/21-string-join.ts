...
type Join<Parts extends string[], Split extends string> = Parts extends [infer First extends string, ...infer Rest extends string[]]
  ? `${ First }${ Rest['length'] extends 0 ? '' : Split }${ Join<Rest, Split> }` : ``;

declare function join<Split extends string>(delimiter: Split):
  <Parts extends string[], Result = []>(...parts: Parts) => Join<Parts, Split>
