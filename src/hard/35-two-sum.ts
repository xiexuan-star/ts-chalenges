...
type ShiftTimes<T extends any[], S extends number, U extends any[] = []> = T extends [never] ? [] : U['length'] extends S ? T : ShiftTimes<Shift<T>, S, [any, ...U]>;

type NumberToArray<T extends number, U extends any[] = []> = U['length'] extends T ? U : NumberToArray<T, [any, ...U]>;

type TwoSum<T extends number[], U extends number> =
  T extends [infer F extends number, ...infer R extends number[]]
    ? Length<ShiftTimes<NumberToArray<U>, F>> extends R[number]
      ? true
      : TwoSum<R, U>
    : false;
